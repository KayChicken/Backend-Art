...
class NewsController {
    async getNews(req,res) {
        try {
            ...
            const news = await db.query("SELECT news_id,news_title,news_desc,news_img,user_name,user_surname,news_date FROM newsline JOIN users ON user_id = news_author WHERE news_id > $1 AND news_id <= $2 ORDER BY news_id DESC LIMIT 10",[currentPage,nextPage])
